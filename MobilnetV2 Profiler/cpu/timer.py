...
class MobileNetV2(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        add_time("Conv2D",("with kernel_size=3, stride=2 and input shape: {}".format(shape),(end-start)*1000))
        ...
        for i in range(len(self.blocks)):
            ...
            add_time("Blocks",("at position {} with input shape: {}".format(i+1,shape),(end-start)*1000),i)
        
        shape = list(x.shape)
        start = time.perf_counter() 
        x = self.conv_last(x)
        end = time.perf_counter() 
        add_time("ConvLast",("with kernel_size=1, stride=1 and input shape: {}".format(shape),(end-start)*1000))
        ...
        add_time("Classifier",("with Dropout=0.2 and dim=1280 input shape: {}".format(shape),(end-start)*1000))
