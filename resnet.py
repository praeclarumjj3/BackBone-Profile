...
if __name__ == '__main__':
    ...
    parser.add_argument("--b", default="bs",
    help="Structure of the residual block ['bs' or 'bt']")
    ...
    if args.i == 'pascal':
        img = torch.randn(int(args.s/2), 3, 500, 334).to("cuda")
    else:
        img = torch.randn(int(args.s/2), 3, 1024, 2048).to("cuda:0")
    ...
    if args.d is 18:
        model = resnet18(block,False).to("cuda:0")
    elif args.d is 34:
        model = resnet34(block,False).to("cuda:0")
    elif args.d is 50:
        model = resnet50(block,False).to("cuda:0")
    elif args.d is 101:
        model = resnet101(block,False).to("cuda:0")
    elif args.d is 152:
        model = resnet152(block,False).to("cuda:0")
    else:
